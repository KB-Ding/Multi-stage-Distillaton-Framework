...
class SentenceTransformer(nn.Sequential):
    ...
    def save_to_hub(self,
        ...
        """
        Uploads all elements of this multilingual Transformer to a new HuggingFace Hub repository.
    ...
    def CT_batching_collate(self, batch):
        """
    ...
    def init_collate(self, collateName, *args, **params):
        collate_list = {
            "smart_batching_collate": self.smart_batching_collate,
            "CT_batching_collate": self.CT_batching_collate
        }

        if collateName in collate_list:
            formatter = collate_list[collateName]

            return formatter
        else:
            logger.info(f"There is no formatter called {collateName}, check your config.")
    ...
    def fit(self,
            ...
            collate_name_list: list = ["smart_batching_collate"]
            ):
        """
        ...
        :param collate_name_list: collate function for each dataloader
        """
        ...
        assert len(collate_name_list) == len(dataloaders)

        # Use smart batching
        for i, dataloader in enumerate(dataloaders):
            dataloader.collate_fn = self.init_collate(collate_name_list[i])
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                if evaluation_steps > 0 and training_steps % evaluation_steps == 0:
                    logger.info(str(training_steps) + " Training Steps Loss: " + str(loss_value.item()))
                ...
                if checkpoint_path is not None and checkpoint_save_steps is not None and checkpoint_save_steps > 0 and global_step % checkpoint_save_steps == 0:
                    logger.info(str(global_step) + " Global Steps Loss: " + str(loss_value.item()))
                    self._save_checkpoint(checkpoint_path, checkpoint_save_total_limit, global_step)

            logger.info(str(epoch) + " Epoch Loss: " + str(loss_value.item()))
            self._eval_during_training(evaluator, output_path, save_best_model, epoch, -1, callback)

        if evaluator is None and output_path is not None:  # No evaluator, but output path: save final model version
            # logger.info("Loss: " + str(loss_value.item()))
