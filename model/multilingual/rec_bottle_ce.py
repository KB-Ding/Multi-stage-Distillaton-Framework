...
class rec_bottle_ce(nn.Module):
    def __init__(self, config, *args, **params):

        super(rec_bottle_ce, self).__init__()
        ...
        emb_model = wrapper_model(hugging_dir)
    ...
    def forward(self, data):
        ...
        simcse_labels = torch.tensor(range(len(scores)), dtype=torch.long,
                                     device=scores.device)  # Example a[i] should match with b[i]

        cot_loss = 0.01 * nn.CrossEntropyLoss()(scores, simcse_labels)
