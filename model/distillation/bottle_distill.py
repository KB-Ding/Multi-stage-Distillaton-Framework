...
class bottle_distill(nn.Module):
    def __init__(self, config, *args, **params):

        super(bottle_distill, self).__init__()
        ...
        auto_model.embeddings = Small_Embeddings(config)
        self.model = student_model

        self.config = config
        self.scale = config.getfloat("model", "scale")
    ...
    def forward(self, data):
        ...
        embeddings_a = \
            self.roberta_model_forward(input_ids=data["en_input_ids"], attention_mask=data["en_attention_mask"])[
                'sentence_embedding']
        embeddings_b = \
            self.roberta_model_forward(input_ids=data["mul_input_ids"], attention_mask=data["mul_attention_mask"])[
                'sentence_embedding']
...
class Small_Embeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.word_embeddings = nn.Embedding(config.getint("model", "vocab_size"), config.getint("model", "embedding_size"), padding_idx=config.getint("model", "pad_token_id"))
        # project layer
        self.word_embeddings_2 = nn.Linear(config.getint("model", "embedding_size"), config.getint("model", "hidden_size"), bias=False)

        self.position_embeddings = nn.Embedding(config.getint("model", "max_position_embeddings"), config.getint("model", "hidden_size"))
        self.token_type_embeddings = nn.Embedding(config.getint("model", "type_vocab_size"), config.getint("model", "hidden_size"))

        self.LayerNorm = nn.LayerNorm(config.getint("model", "hidden_size"), eps=config.getfloat("model", "layer_norm_eps"))
        self.dropout = nn.Dropout(config.getfloat("model", "hidden_dropout_prob"))

        self.register_buffer("position_ids", torch.arange(config.getint("model", "max_position_embeddings")).expand((1, -1)))
        self.position_embedding_type = "absolute"

        self.padding_idx = config.getint("model", "pad_token_id")
        self.position_embeddings = nn.Embedding(
            config.getint("model", "max_position_embeddings"), config.getint("model", "hidden_size"), padding_idx=self.padding_idx
        )
