...
class rec_bottle_bool(nn.Module):
    def __init__(self, config, *args, **params):

        super(rec_bottle_bool, self).__init__()
    ...
    def forward(self, data):
        ...
        label_scores = torch.tensor([1] * len(scores), dtype=torch.float,
                                    device=scores.device)  # Example a[i] should match with b[i]

        label_scores = torch.diag_embed(label_scores)
