...
class wrapper_model(nn.Module):
    def __init__(self, hugging_dir):
        super(wrapper_model, self).__init__()
        self.model = SentenceTransformer(hugging_dir)

class align_bottle_mse(nn.Module):
    def __init__(self, config, *args, **params):

        super(align_bottle_mse, self).__init__()

        hugging_dir = os.path.join(config.get("model", "student_model_path"))
        student_model = SentenceTransformer(hugging_dir)

        emb_model = wrapper_model(hugging_dir)  # ._first_module().auto_model.embeddings
        emb_model.model._first_module().auto_model.embeddings = Small_Embeddings(config)
        emb_model.model._first_module().auto_model.encoder.layer = torch.nn.ModuleList([])
        emb_model.model._first_module().auto_model.config.num_hidden_layers = 0
        ...
        # emb_model = wrapper_model(hugging_dir)  # ._first_module().auto_model.embeddings
        student_model._first_module().auto_model.embeddings = copy.deepcopy(emb_model.model._first_module().auto_model.embeddings) #= Small_Embeddings(config)

        new_layers = torch.nn.ModuleList(
            [layer_module for i, layer_module in enumerate(student_model._first_module().auto_model.encoder.layer) if
             i in layers_to_keep])
        student_model._first_module().auto_model.encoder.layer = new_layers

        student_model._first_module().auto_model.config.num_hidden_layers = len(layers_to_keep)
