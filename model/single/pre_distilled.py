...
class pre_distilled(nn.Module):
    def __init__(self, config, *args, **params):

        super(pre_distilled, self).__init__()

        hugging_dir = os.path.join(config.get("model", "temp_student_path"))
        ...
        param = torch.load(config.get("model", "student_model_path"), map_location=torch.device('cpu'))
