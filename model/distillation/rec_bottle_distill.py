...
class rec_bottle_distill(nn.Module):
    def __init__(self, config, *args, **params):

        super(rec_bottle_distill, self).__init__()
        ...
        emb_model = wrappermodel(hugging_dir)  # ._first_module().auto_model.embeddings
        ...
        emb_model.model._first_module().auto_model.encoder.layer = torch.nn.ModuleList(
            [layer_module for i, layer_module in enumerate(auto_model.encoder.layer) if i in []])
        ...
        auto_model.embeddings = emb_model.model._first_module().auto_model.embeddings
