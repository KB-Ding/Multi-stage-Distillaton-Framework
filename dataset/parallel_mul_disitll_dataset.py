...
class parallel_separate_dataset(Dataset):
    ...
    def __init__(self, config, mode, encoding="utf8"):
        ...
        self.teacher_model = SentenceTransformer(modules=[word_embedding_model, pooling_model])
    ...
    def generate_data(self):
        ...
        for src_embedding, trg_sentences in zip(src_embeddings, target_sentences_list):
            ...
            for i in range(0, len(trg_sentences) - 1, 2):
                self.cache.append(parallel_input(texts=[trg_sentences[i], trg_sentences[i + 1]], label=src_embedding))
